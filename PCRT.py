...
class PNG(object):
    ...
    def MakeAncillary(self, name, payload):
        ...
        data = struct.pack('!I4s%dsI' % (length), length,
                           name.encode(), payload.encode(), crc)
        return data
    ...
    def FindAncillary(self, data):
        ...
        for text in attach_txt:
            ...
            while pos != -1:
                ...
                if pos != -1:
                    length = bytes2num(data[pos-4:pos])
    ...
    def CheckIHDR(self, data):
        ...
        if calc_crc != None:
            print((Termcolor('Detected', 'Error IHDR CRC found! (offset: %s)\nchunk crc: %s\ncorrect crc: %s' % (
                int2hex(pos+4+length), bytes2hex(crc), bytes2hex(calc_crc)))))
            if self.choices != '':
                choice = self.choices
            else:
                msg = Termcolor(
            ...
            print(('[Finished] Correct IHDR CRC (offset: %s): %s' %
                  (int2hex(pos+4+length), bytes2hex(crc))))
    ...
    def CheckIDAT(self, data):
        ...
        IDAT_table = []
        idat_begin = data.find(b'\x49\x44\x41\x54')-4
        if idat_begin == -1:
            ...
            if length != len(chunk_data):
                ...
                if calc_crc != None:
                    print((Termcolor('Detected', 'Error IDAT CRC found! (offset: %s)\nchunk crc: %s\ncorrect crc: %s' % (
                        int2hex(offset+8+length), bytes2hex(crc), bytes2hex(calc_crc)))))
                    if self.choices != '':
                        choice = self.choices
                    else:
                        msg = Termcolor(
                    ...
                    print(('[Finished] Correct IDAT CRC (offset: %s): %s' %
                          (int2hex(offset+8+length), bytes2hex(crc))))
