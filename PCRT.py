...
def str2num(s, n=0):
    if n == 4:
        ...
        return int(str2hex(s.encode()), 16)
...
class PNG(object):
    ...
    def GetPicInfo(self, ihdr=''):
        '''
        bits: color depth
        mode: 0:gray[1] 2:RGB[3] 3:Indexed[1](with palette)
        4:grey & alpha[2] 6:RGBA[4]
        compression: DEFLATE(LZ77+Huffman)
        filter: 0:None 1:sub X-A 2:up X-B 3:average
        X-(A+B)/2 4:Paeth p = A + B − C
        C B D
        A X
        '''
        data = self.LoadPNG()
        ...
        self.width, self.height, self.bits, self.mode, self.compression, \
            self.filter, self.interlace = struct.unpack('!iiBBBBB', ihdr)
    ...
    def PrintPicInfo(self):
        ...
        mode_dict = {0: 'Grayscale', 2: 'RGB', 3: 'Indexed',
    ...
    def DecompressPNG(self, data, channel=3, bits=8, width=1, height=1):
        ...
        if width == 0 and height == 0:
            ...
            for bits in [8, 16]:
                for channel in [4, 3, 1, 2]:
                    ...
                    for i in range(0, len(size_list), 2):
                        ...
                        tmp = self.ClearFilter(
                            z_idat, width, height, channel, bits)
                        if tmp != -1:
                            img = Image.fromstring(
                                channel_dict[channel], (width, height), tmp)
                            # img.show()
                            filename = PATH + \
                                'test(%dx%d)_%dbits_%dchannel.png' % (
                                    width, height, bits, channel)
                            img.save(filename)
            ...
            img = Image.fromstring(
                mode_dict[self.mode], (width, height), z_idat)
            img.show()
            img.save('zlib.png')
    ...
    def FindAncillary(self, data):
        # ancillary = ['cHRM', 'gAMA', 'sBIT', 'PLTE', 'bKGD', 'sTER', 'hIST', 'iCCP', 'pHYs', 'sPLT', 'sRGB',
        #             'dSIG', 'eXIf', 'iTXt', 'tEXt', 'zTXt', 'tIME', 'tRNS', 'oFFs', 'sCAL', 'fRAc', 'gIFg', 'gIFt', 'gIFx']
    ...
    def CheckIHDR(self, data):
        ...
        if calc_crc is not None:
            ...
            if choice == 'y' or choice == '':
                if width > height:
                    ...
                    for h in range(height, width):
                        chunk_ihdr = IHDR[8:12] + \
                            struct.pack('!I', h)+IHDR[16:8+length]
            ...
            print('[Finished] Correct IHDR CRC (offset: %s): %s' %
                  (int2hex(pos+4+length), str2hex(crc)))
        self.file.write(IHDR)
        print('[Finished] IHDR chunk check complete (offset: %s)' %
              (int2hex(pos-4)))
    ...
    def CheckIDAT(self, data):
        ...
        if self.i_mode == 0:
            ...
            idat_size = struct.unpack(
                '!I', data[idat_begin:idat_begin+4])[0]+12
            ...
            if pos_IEND != -1:
                pos_list = [g.start() for g in re.finditer(
                    b'IDAT', data) if g.start() < pos_IEND]
            else:
        ...
        for IDAT in IDAT_table:
            ...
            if length != len(chunk_data):
                print(Termcolor(
                    'Detected', 'Error IDAT chunk data length! (offset: %s)' % (int2hex(offset))))
                ...
                if choice == 'y' or choice == '':
                    ...
                    if chunk_data is None:
                        print(
                            Termcolor('Failed', 'Fixing failed, auto discard this operation...'))
                        chunk_data = IDAT[8:-4]
                    else:
                        IDAT = IDAT[:8]+chunk_data+IDAT[-4:]
                        print('[Finished] Successfully recover IDAT chunk data')
            else:
                print('[Finished] Correct IDAT chunk data length (offset: %s length: %s)' % (
                ...
                if calc_crc is not None:
                    print(Termcolor('Detected', 'Error IDAT CRC found! (offset: %s)\nchunk crc: %s\ncorrect crc: %s' % (
                        int2hex(offset+8+length), str2hex(crc), str2hex(calc_crc))))
                    if self.choices != '':
                        choice = self.choices
                    else:
                        msg = Termcolor(
                    ...
                    print('[Finished] Correct IDAT CRC (offset: %s): %s' %
        ...
        print('[Finished] IDAT chunk check complete (offset: %s)' %
              (int2hex(idat_begin)))
    ...
    def CheckIEND(self, data):
        ...
        if pos == -1:
            ...
            if data[pos+8:] != '':
                print(Termcolor('Detected', 'Some data (length: %d) append in the end (%s)' % (
                ...
                while True:
                    ...
                    if choice == '1':
                        ...
                        print('data:', data[pos+8:].decode())
                        print('hex(data):', binascii.hexlify(data[pos+8:]).decode())
...
Version: %s
    ...
    parser.add_argument(
        '-y', '--yes', help='auto choose yes', action='store_true')
    parser.add_argument(
        '-v', '--verbose', help='use the safe way to recover',
        action='store_true')
    parser.add_argument('-m', '--message',
                        help='show the image information', action='store_true')
    parser.add_argument('-n', '--name', help="payload name [Default: random]")
    parser.add_argument('-p', '--payload', help="payload to hide")
    parser.add_argument('-w', '--way', type=int, default=1,
                        help="payload chunk: [1]: ancillary [2]: critical [Default:1]")

    parser.add_argument('-d', '--decompress',
                        help="decompress zlib data file name")

    parser.add_argument(
        '-i', '--input', help='Input file name (*.png) [Select from terminal]')
    parser.add_argument(
        '-f', '--file', help='Input file name (*.png) [Select from window]', action='store_true')
